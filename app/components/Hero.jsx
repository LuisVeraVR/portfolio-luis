...
function Hero() {
  ...
  return (
    <div className="w-11/12 max-w-3xl text-center mx-auto h-screen flex flex-col items-center justify-center gap-4">
      <div>
        <Image src={assets.user_image || "/placeholder.svg"} alt="" className="rounded-full w-32" />
      </div>
      <h3 className="flex items-end gap-2 text-xl md:text-2xl mb-3 font-Ovo dark:text-white">
        {t.greeting} <Image src={assets.hand_icon || "/placeholder.svg"} alt="" className="w-6" />
      </h3>
      <h1 className="text-3xl sm:text-6xl lg:text-[66px] font-Ovo dark:text-white">{t.title}</h1>
      <p className="max-w-2xl mx-auto font-Ovo dark:text-gray-300">{t.description}</p>
